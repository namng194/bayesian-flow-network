...
def final_comparison_table():
    ...
    ax.axhline(1.0 - row_h, color="black", lw=1.0)
    ...
    ax.axhline(row_h * 0.5, color="black", lw=0.5)
    ax.set_title(
...
def final_speed_quality():
    ...
    if bfn_json.exists():
        with open(bfn_json) as f:
            data = json.load(f)
        # Real key is "bits_estimate" (nats loss already converted to bits),
        # not "bpc". Skip runs that errored or produced no estimate.
        pts = [
            (r["n_steps"], r["bits_estimate"])
            for r in data["results"]
            if not r.get("error")
            and r.get("bits_estimate") is not None
            and math.isfinite(r["bits_estimate"])
            and r["n_steps"] > 0
        ]
    ...
    if ddpm_json.exists():
        ...
        plotted_any = False
        if bench.get("bfn"):
            series = sorted(bench["bfn"], key=lambda r: r["n_steps"])
            xs = [r["n_steps"] for r in series]
            ys = [r["ms_per_sample"] for r in series]
            ax2.plot(xs, ys, "o-", color=COLORS["bfn"], lw=2.2, label="BFN")
            plotted_any = True
 
        # Don't assume the baseline's key name (e.g. "ddpm_ddim") — discover
        # any other list-of-dicts entry in the file that looks like a timing
        # series and plot it too.
        meta_keys = {"dataset", "device", "gpu_name", "n_samples", "bfn"}
        for key in bench.keys():
            if key in meta_keys:
                continue
            series = bench[key]
            if isinstance(series, list) and series and "ms_per_sample" in series[0]:
                series = sorted(series, key=lambda r: r["n_steps"])
                xs = [r["n_steps"] for r in series]
                ys = [r["ms_per_sample"] for r in series]
                ax2.plot(xs, ys, "s--", color=COLORS["ddpm"], lw=2.0,
                          label=key.replace("_", "/").upper())
                plotted_any = True
 
        if not plotted_any:
            ddpm_json = None  # fall through to theoretical estimate below
    if not ddpm_json.exists() if isinstance(ddpm_json, Path) else True:
        pass
    if ddpm_json is None or not ddpm_json.exists():
